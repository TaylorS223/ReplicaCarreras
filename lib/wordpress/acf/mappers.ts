...
import type { PlanEstudiosContent, StudyLevel, Course } from "@/types/api";
...
const getSilaboUrl = async (value: number | string | { url?: string } | undefined): Promise<string> => {
  ...
  // Es un ID numérico — resuelve via Media API
...
export const mergeCarreraFromInicioPagina = async (
  ...
  // Hero — imagen y texto del banner
  if (acf.bannerimagen || acf.bannerimagentexto) {
    const imagenUrl = acf.bannerimagen
      ? await resolveMediaUrl(acf.bannerimagen)
      : "";
    ...
    result.hero = {
      ...
      ...(imagenUrl
        ? { images: [{ src: imagenUrl, alt: acf.bannerimagentexto ?? "" }] }
        : {}),
    };
  }
  ...
  // Eslogan motivacional → sectionTitle del perfil
  if (acf.esloganmotivacional) {
    result.profile = {
      ...result.profile,
      sectionTitle: acf.esloganmotivacional,
    };
  }

  // Perfil de egreso → primera card del perfil
  ...
  // Campo laboral → segunda card del perfil
...
export const mapNoticiaPostToProyecto = async (
  ...
  const fecha = post.acf?.fechanotifica ?? "";
