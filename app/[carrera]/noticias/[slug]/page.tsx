...
export async function generateMetadata({ params }: NoticiaDetailPageProps): Promise<Metadata> {
  ...
  const noticias = getNoticias({ facultadSlug: carrera, carreraSlug: carrera });
  const noticia = noticias.find((n) => n.slug === slug);
  if (!noticia) return { title: "Noticia" };
  return {
    title: `${noticia.titulo} | ${carrera}`,
    ...
    <>
      ...
      {(anteriorNoticia || siguienteNoticia) && (
        <div className="nd-post-nav">
          {anteriorNoticia ? (
            <Link
              href={`/${carrera}/noticias/${anteriorNoticia.slug}`}
          ...
          {siguienteNoticia ? (
            <Link
              href={`/${carrera}/noticias/${siguienteNoticia.slug}`}
