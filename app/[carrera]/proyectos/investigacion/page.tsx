...
import { getProyectosInvestigacion } from "@/lib/wordpress/graphql/proyectos";
...
export default async function InvestigacionPage({ params }: Props) {
  ...
  const proyectos = await getProyectosInvestigacion(carrera);
    ...
    <div className="container" style={{ padding: "64px 0 80px" }}>
      ...
      {proyectos.length === 0 ? (
        <p style={{ color: "#8a9ab5", fontSize: "1rem" }}>
          Próximamente publicaremos los proyectos de investigación.
        </p>
      ) : (
        proyectos.map((p, i) => (
          <article
