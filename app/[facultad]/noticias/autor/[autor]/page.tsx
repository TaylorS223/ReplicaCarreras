...
type AutorPageProps = {
  params: Promise<{ facultad: string; autor: string }>;
};

export default async function AutorPage({ params }: AutorPageProps) {
  const { facultad, autor } = await params;
  ...
  const noticias = todasNoticias.filter((n) => n.autor === autor);
    ...
    <div className="ap-wrapper">
      <div className="ap-page-title">
        <div className="container">
          <div className="ap-page-caption">{autor}</div>
          <h3 className="ap-page-heading">By</h3>
        </div>
      </div>
      ...
      <div className="container ap-body">
        <main className="ap-main">
          {noticias.length === 0 ? (
            <p className="ap-empty">No se encontraron noticias de este autor.</p>
          ) : (
